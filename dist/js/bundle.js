...
var _vueLs = __webpack_require__(8);

var _vueLs2 = _interopRequireDefault(_vueLs);
...
new _vue2.default({
  ...
  beforeMount: function beforeMount() {
    if (_vue2.default.ls.get('lsValue')) {
      this.lists = JSON.parse(_vue2.default.ls.get('lsValue'));
    }
  },
  methods: {
    // 入力欄表示
    ...
    // 完了と未完了切替
    toggleDone: function toggleDone(list) {
  ...
  computed: {
    ...
    searchLists: function searchLists() {
      ...
      return this.lists.filter(function (elm) {
        var regexp = new RegExp('^' + _this.searchWord, 'i');
  ...
  watch: {
    lists: {
      ...
      handler: function handler(lists) {
        _vue2.default.ls.set('lsValue', JSON.stringify(lists), 60 * 60 * 1000);
      },
